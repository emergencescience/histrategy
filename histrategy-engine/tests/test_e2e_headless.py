...
def scenario_1_liubei_historical():
    ...
    log.append("Histrategy E2E — Scenario 1: 刘备历史路线")
    ...
    for turn_num in range(1, 60):
        ...
        for p in proposals:
            log.append(
                f"  [Event] {world.year}年{world.season.cn} — {p.title}: {p.narrative_hint[:60]}"
            )
...
def _get_liubei_historical_commands(world: WorldState) -> list[Command]:
    ...
    if year <= 208:
        if faction.food < 3000 and faction.territories:
            cmds.append(
                Command(
                    type="develop", params={"territory": faction.territories[0]}, faction_id="shu"
                )
            )
...
def scenario_2_liubei_alt_jingzhou():
    ...
    factions = {
        "shu": FactionState(
            ...
            ruler_id="liubei",
            capital="xiangyang",
            territories=["xiangyang", "jiangling", "xinye"],
            strength_actual=20000,
            treasury=8000,
            food=10000,
            tax_rate=0.25,
            morale_actual=80,
    ...
    for turn_num in range(1, 20):
        ...
        for p in proposals:
            log.append(
                f"  [AltEvent] {world.year}年{world.season.cn} — {p.title}: {p.narrative_hint[:60]}"
            )
...
def _get_jingzhou_focus_commands(world: WorldState) -> list[Command]:
    ...
    if faction.territories:
        cmds.append(
            Command(type="develop", params={"territory": faction.territories[0]}, faction_id="shu")
        )
    ...
    for tid in faction.territories:
        ...
        for nid in t.neighbors:
            ...
            if nt and nt.owner_id not in ("", "cao", faction.id):
                ...
                cmds.append(
                    Command(type="attack", params={"target_territory": nid}, faction_id="cao")
                )
                return cmds

    if faction.territories:
        cmds.append(
            Command(type="develop", params={"territory": faction.territories[0]}, faction_id="cao")
        )
    return cmds
...
def scenario_4_sunquan_redcliffs():
    ...
    world = WorldState(
        ...
        turn_number=1,
        scenario="wu_redcliffs",
        player_faction_id="wu",
        territories=territories,
        characters=characters,
        factions=factions,
        armies=armies,
    ...
    for tid in faction.territories:
        ...
        for nid in t.neighbors:
            ...
            if nt and nt.owner_id == "cao":
                cmds.append(
                    Command(type="attack", params={"target_territory": nid}, faction_id="wu")
                )
                return cmds

    if faction.territories:
        cmds.append(
            Command(type="develop", params={"territory": faction.territories[0]}, faction_id="wu")
        )
...
def test_liubei_historical_207_223():
    ...
    log.append("Histrategy E2E — Scenario 5: 刘备史实增强版")
    log.append(f"起始: 公元{world.year}年{world.season.cn}")
    ...
    for turn_num in range(1, 65):
        ...
        for p in proposals:
            log.append(
                f"  [Event] {world.year}年{world.season.cn} — {p.title}: {p.narrative_hint[:80]}"
            )
            ...
            if "刘表" in p.title and "死" in p.title:
                ...
                if "xiangyang" in world.territories:
                    world.territories["xiangyang"].owner_id = "cao"
                    if (
                        "cao" in world.factions
                        and "xiangyang" not in world.factions["cao"].territories
                    ):
        ...
        for ce in result.character_events:
            if "death" in str(ce.get("type", "")):
                log.append(
                    f"  [Death] {ce.get('character_name', '?')} 去世于{world.year}年{world.season.cn}"
                )
    ...
    for tid in faction.territories:
        ...
        for nid in t.neighbors:
            ...
            if nt and nt.owner_id == "cao":
                ...
                cmds.append(Command(type="move", params={"destination": nid}, faction_id="wu"))
                return cmds
