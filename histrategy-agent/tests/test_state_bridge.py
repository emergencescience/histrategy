...
from histrategy_engine import Command, UnitType
...
class TestStateBridgeCommands:
    ...
    def test_execute_info(self):
        result = self.bridge.execute_command(
            Command(type="info", params={}, faction_id="shu")
        )
    ...
    def test_execute_recruit_success(self):
        ...
        total = sum(
            a.total_troops
            for a in self.session.world_state.armies.values()
            if a.faction_id == "shu"
        )
        assert total == 5500
    ...
    def test_execute_move_no_target(self):
        result = self.bridge.execute_command(
            Command(type="move", params={}, faction_id="shu")
        )
        assert result["success"] is False
    ...
    def test_execute_attack_no_target(self):
        result = self.bridge.execute_command(
            Command(type="attack", params={}, faction_id="shu")
        )
        assert result["success"] is False
        assert "指定" in result["message"]
    ...
    def test_execute_develop(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = bridge.execute_command(
                Command(type="develop", params={"target": "xuchang"}, faction_id="cao")
            )
            assert result["success"] is True
    ...
    def test_execute_tax(self):
        ...
        result = self.bridge.execute_command(
            Command(type="tax", params={"rate": 0.3}, faction_id="shu")
        )
        assert result["success"] is True
    ...
    def test_execute_tax_invalid_rate(self):
        result = self.bridge.execute_command(
            Command(type="tax", params={"rate": 0.9}, faction_id="shu")
        )
        assert result["success"] is False
    ...
    def test_execute_unknown_command(self):
        result = self.bridge.execute_command(
            Command(type="dance", params={}, faction_id="shu")
        )
        assert result["success"] is False
    ...
    def test_execute_info_gives_snapshot(self):
        result = self.bridge.execute_command(
            Command(type="info", params={}, faction_id="cao")
        )
        assert result["success"] is True
...
class TestStateBridgeEdgeCases:
    ...
    def test_nonexistent_faction_commands(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = bridge.execute_command(
                Command(type="recruit", params={"amount": 100}, faction_id="nobody")
            )
            assert result["success"] is False
