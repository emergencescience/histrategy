...
class ServerClient:
    ...
    def submit_room_decision(
        ...
        """Submit a decision in a multiplayer room.

        Args:
            room_id: Room ID
            faction_id: Faction display name (e.g. "caocao", "liubei")
            user_id: User ID from enter_room
            decision: Player decision text
            player_token: Player token for pre-assigned rooms

        Returns:
            Dict with ok, status, submitted, pending
        """
        body: dict[str, Any] = {
        ...
        return self._post(f"/api/rooms/{room_id}/decide", body)

    def get_room_status(self, room_id: str, faction_id: str = "") -> dict:
        """Get room status.

        Args:
            room_id: Room ID
            faction_id: Optional faction to get narrative for

        Returns:
            Dict with ok, room_id, phase, quarter, submitted, pending, slots, players
        """
        params = {}
        if faction_id:
            params["faction_id"] = faction_id
        return self._get(f"/api/rooms/{room_id}/status", params=params)

    def get_room_turns(self, room_id: str) -> dict:
        """Get quarter turn history for a room.

        Returns:
            Dict with room_id, turns (list), count
        """
        return self._get(f"/api/rooms/{room_id}/turns")

    def get_room_state(self, room_id: str) -> dict:
        """Get game state for a room.

        Returns:
            Dict with room_id, quarter_number, factions
        """
        return self._get(f"/api/rooms/{room_id}/state")
