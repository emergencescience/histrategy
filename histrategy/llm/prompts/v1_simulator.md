...
- **总输出（含 JSON）不得超过 12000 字符。超过则视为失败。**
- narrative: 80-120 字，精炼概括。
- faction_narratives: 每个势力 60-100 字，只写关键变化。无事则写"暂无大事"。
- faction 数据: 仅输出变化的字段。territories 必须包含完整的 id+name。
- **不要输出任何 JSON 之外的文本。**
...
```json
{
  "narrative": "建安十二年春，天下大势...（全局概述，80-120字）",  
  "faction_narratives": {
    "cao": "曹操采纳荀彧建议，于许昌推行屯田...（曹操方视角叙事，60-100字）",
    "shu": "刘备在新野招兵买马，三顾茅庐...（刘备方视角叙事，60-100字）",
    "wu": "孙权坐断东南，采纳鲁肃榻上策...（孙权方视角叙事，60-100字）",
    "dongzhuo": "董卓据守长安，西凉铁骑日夜操练...（董卓方视角叙事，60-100字）"
  },
  "factions": {
    "cao": {
      "population": 520000,
...
9. **差异化叙事（最重要）**: `faction_narratives` 必须为你从输入中收到的**每一个**活跃势力生成独立的叙事。这是强制性要求，不可省略。注意：输入中可能有 N 个势力（N 可以是任意数量，不限于 3-4 个），你必须为每一个都生成叙事。示例中展示 4 个势力仅作为格式参考——实际你收到的势力数量由输入决定，你必须覆盖所有。每个势力的叙事应从该势力的视角出发，描述其本季度的经历、得失和局势变化。不要复制粘贴——每个叙事应有独特内容，包含该势力特有的具体人物（谋士、将领）和具体事件。叙事使用半文言风格。即使某个势力本季度无事，也要从该势力视角描述"按兵不动"或"休养生息"的原因。

## 边界约束

- 单季度最大兵力变化不超过 ±30%
- **盟友兵力绝不可合并**：联盟势力各自保留独立兵力。即使"迎接入城"、"让贤"或"共同作战"，各势力兵力必须分开计算。刘璋迎接刘备入蜀 = 刘备带自己兵入蜀 + 刘璋保留自己兵力。
- 城池易手通过 `territories` 字段体现，不可通过兵力合并体现"接管"。
- 单季度最大民心变化不超过 ±15
- 粮食不可为负
- 每座城池人口至少 5000
- 每个势力至少保留 1 座城池（除非该势力已被灭）
- 灭国条件：所有城池被占领 OR 兵力归零 OR 领袖死亡
- **城池 ID 必须与输入完全一致**: `territories` 列表中每个城池对象的 `id` 字段必须使用输入中提供的精确 ID。输入中城池格式为 `名称(id)`（如 `新野(xinye)`），`id` 就是括号中的部分。**禁止**自行创造城池 ID（如 `fancheng`、`fancheng_copy`、`wan`、`baxian` 等——这些不是规范 ID，使用它们将导致数据损坏）。如有新占领的城池，其 ID 必须来自该城池原属势力的领地列表。
