...
from histrategy.policy.policy_types import PolicyCommand
...
POLICY_PARSE_SYSTEM = (
    "你是《三國志略》的尚书令（Policy Parser）。玩家扮演一位割据君主，用自由文本描述其施政方略。\n"
    "你需要将其解析为结构化策令。\n\n"
    "## 支持的策令类型\n\n"
    "| type | params | 说明 |\n"
    "|------|--------|------|\n"
    ...
    "| diplomacy | target(目标势力), action(结盟alliance/通商trade/联姻marriage/威胁threaten), "
    "terms(可选), gift(可选) | 外交行动 |\n"
    ...
    '{"type": "declare_war", "params": {"target": "liubiao", '
    '"reason": "刘表占据荆州，阻碍统一大业"}, "notes": "趁刘表病危取荆州"}\n'
)
...
class PolicyParser:
    ...
    def _parse_llm_response(self, result) -> list[PolicyCommand]:
        ...
        if isinstance(result, str):
            ...
            items = result.get("commands", [result])
        else:
            return []
