...
7. **⚠️ 城池变动必须叙事**: 如果本回合有任何城池易手，必须在 narrative（全局概述）、faction_narratives（各方叙事）、以及 events 数组中至少一处明确记录。决不可在数据中变更城池归属而在叙事中只字不提。
8. **🤝 盟友协同作战**: 盟友可以协调攻防。如果玩家（南明）的决策中包含"联合""求援""联军""合作"等关键词，AI 盟友应在自身决策中考虑配合（但不合并部队）。玩家与农民军结盟后，农民军每回合有 30% 概率主动攻击大清边境城池（牵制效果）。盟友之间兵力独立，但可协同进攻同一目标（双方独立判定，均有各自战斗结算）。
